...
export const useCompletion = () => {
  ...
  const submit = useCallback(
    async (
      ...
      transientNotice: string | null = null
    ) => {
      const input = speechText || state.input;
      const activeFiles = transientFiles.length
        ? transientFiles
        : state.attachedFiles;
      ...
      try {
        ...
        if (!selectedAIProvider.provider && !useLocalAPI) {
          setState((prev) => ({
            ...
            notice: null,
            error:
              "No AI provider is configured. Open Dev Space and add your API key and model name.",
          }));
          return;
        }
        ...
        if (!provider && !useLocalAPI) {
          setState((prev) => ({
            ...
            isLoading: false,
            notice: null,
            error: "The selected AI provider is no longer available.",
          }));
          return;
        }
  ...
  const saveCurrentConversation = useCallback(
    async (
      ...
      const userMsg: ChatMessage = {
        ...
        timestamp,
        attachedFiles:
          _attachedFiles.length > 0 ? [..._attachedFiles] : undefined,
      };
        ...
        if (prompt) {
          const attachedFile: AttachedFile | null =
            payload.kind === "image"
              ...
              : null;
          const userMessage =
            payload.kind === "ocr_text"
              ? buildOcrUserMessage(prompt, [payload.context])
              : prompt;
          const imagesBase64 =
            payload.kind === "image" ? [payload.imageBase64] : [];
  ...
  const handleUltraInstinctCapture = useCallback(
    async (prompt?: string) => {
      ...
      if (prompt) {
        await submit(prompt, [], [payload.context], payload.notice);
        return;
      }
  ...
  }, [micOpen, messageHistoryOpen, resizeWindow, isFilesPopoverOpen]);

  useEffect(() => {
    const syncResponseWindow = async () => {
      if (!isPopoverOpen) {
        await invoke("hide_response_window");
        return;
      }

      await invoke("update_response_window", {
        state: {
          open: true,
    ...
    syncResponseWindow().catch((error) => {
      console.error("Failed to sync response window:", error);
    });
  }, [
    isPopoverOpen,
    state.isLoading,
  ...
  useEffect(() => {
    ...
    listen("response-window-action", (event: any) => {
      const action = event.payload?.action;
      if (action === "set_keep_engaged") {
        setKeepEngaged(Boolean(event.payload?.checked));
        return;
      }
  ...
  const captureScreenshot = useCallback(async () => {
    ...
    setState((prev) => ({
      ...
      response: "",
      error: null,
      notice: config.ultraInstinctEnabled
        ? "Phantom Ultra Instinct Mode is reading active-window text..."
        : config.enabled
  ...
  }, [handleScreenshotSubmit, handleUltraInstinctCapture, resolveScreenshotPrompt]);
