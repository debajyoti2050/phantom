...
export const PromptLibrary = () => {
  ...
  // Watch for changes in user's selected prompt and clear bundled prompt selection if needed
  useEffect(() => {
  ...
  const fetchPromptLibrary = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch Phantom prompts:", err);
      setError(
        typeof err === "string" ? err : "Failed to fetch Phantom prompts"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectPromptLibraryItem = async (prompt: PromptLibraryItem) => {
    try {
      ...
      console.error("Failed to select Phantom prompt:", error);
    }
  };
    ...
    return (
      <div className="space-y-4 mt-6">
        ...
        <Empty
          ...
          description="Fetching Phantom default prompts"
        />
      </div>
    );
  }
