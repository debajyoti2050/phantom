...
function applyOverlayAlwaysOnTop(win, enabled = true) {
  if (!win || win.isDestroyed()) return;
  win.setAlwaysOnTop(Boolean(enabled), process.platform === "darwin" ? "screen-saver" : "normal");
...
function createMainWindow() {
  ...
  applyOverlayAlwaysOnTop(mainWindow, true);
...
function updateResponseWindow(state) {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    applyOverlayAlwaysOnTop(win, true);
  }
...
async function handleInvoke(command, args = {}) {
  switch (command) {
    ...
    case "set_always_on_top":
      if (mainWindow && !mainWindow.isDestroyed()) {
        applyOverlayAlwaysOnTop(mainWindow, Boolean(args.enabled));
      }
      if (responseWindow && !responseWindow.isDestroyed()) {
        applyOverlayAlwaysOnTop(responseWindow, true);
      }
      return null;
    case "exit_app":
      app.quit();
